...
const App: React.FC = () => {
  ...
  const handleGenderDetection = async (image: ImageData) => {
      if (!isProUser) {
          const cost = 1;
          const currentCredits = profile?.credits ?? parseInt(localStorage.getItem('guestCredits') || '3', 10);
      ...
      try {
          ...
          if (!isProUser) {
            const newCredits = (profile?.credits ?? 3) - 1;
            if (user && profile) await updateProfile({ credits: newCredits });
            else localStorage.setItem('guestCredits', newCredits.toString());
          }
      } catch (err) {
  ...
  const executeImageEdit = async (cost: number, fullPrompt: string) => {
    ...
    try {
      ...
      if (!isProUser) {
          const newCredits = (profile?.credits ?? 3) - cost;
          if (user && profile) await updateProfile({ credits: newCredits }); else localStorage.setItem('guestCredits', newCredits.toString());
      }
  ...
  const handlePaymentSuccess = async (creditsToAdd: number) => {
    ...
    if (user && profile) await updateProfile({ credits: profile.credits + creditsToAdd });
    else { setShowPurchaseModal(false); setShowAuthModal(true); setError("Please sign in to add credits to your account."); return; }
  ...
  const handleProSubscriptionSuccess = async () => {
    setPaymentStatus('success');
    if (user && profile) await updateProfile({ is_pro: true });
    else { setShowPurchaseModal(false); setShowAuthModal(true); setError("Please sign in to activate your Pro subscription."); return; }
