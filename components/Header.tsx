...
const Header: React.FC<HeaderProps> = ({ onBuyCreditsClick, onAuthClick }) => {
  const { user, profile, signOut, loading } = useAuth();
  const isProUser = profile?.is_pro ?? false;
  const credits = profile?.credits ?? (user ? 0 : 3);
        ...
        <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 bg-gray-900/50 px-3 py-1.5 rounded-lg border border-gray-700">
                {isProUser ? (
                    ...
                    <>
                        <span className="font-bold text-lg text-yellow-400">{loading && !profile ? '...' : credits}</span>
