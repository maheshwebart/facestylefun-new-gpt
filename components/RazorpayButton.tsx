...
const RazorpayButton: React.FC<RazorpayButtonProps> = ({ amount, description, onSuccess, onError, disabled, onAuthRequest }) => {
  const { user } = useAuth();
  ...
  const handlePayment = () => {
    const options = {
      key: RAZORPAY_KEY_ID,
      amount: parseFloat(amount) * 100, // Amount in cents for USD
      currency: "USD",
      name: "facestyle.fun",
  ...
  return (
    <Button
      onClick={onAuthRequest ? clickHandler : handlePayment}
      disabled={disabled || !RAZORPAY_KEY_ID}
      className="!bg-sky-600 hover:!bg-sky-700 focus:ring-sky-400 w-full"
    >
