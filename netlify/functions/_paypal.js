...
function resolveCreds() {
  if (ENV === "live") {
    ...
    const clientId =
      ...
      process.env.PAYPAL_SANBOX_CLIENT_ID || ""; // tolerates your typo SANBOX
    const secret =
...
export async function getAccessToken() {
  ...
  if (!clientId || !secret) {
    throw new Error(
      `Missing PayPal ${ENV.toUpperCase()} credentials. Ensure PAYPAL_${ENV === "live" ? "LIVE" : "SANDBOX"}_CLIENT_ID and _SECRET are set.`
    );
  }
